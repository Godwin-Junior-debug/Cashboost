...
import { ArrowLeft, Loader2, UploadCloud, FileImage, X, CheckCircle2, User, Hash, Clock3 } from 'lucide-react';
...
export default function ConfirmPaymentPage({ actionLoading, submitted, isAdmin = false, onSubmit, onBack }: ConfirmPaymentPageProps) {
    ...
    return (
      <div className="space-y-6 animate-fade-in">
        ...
        <div className="bg-white rounded-2xl p-8 sm:p-10 border border-slate-200 text-center">
          <div className={`w-16 h-16 rounded-2xl ${isAdmin ? 'bg-green-50' : 'bg-amber-50'} flex items-center justify-center mx-auto mb-4`}>
            {isAdmin ? <CheckCircle2 className="w-8 h-8 text-green-600" /> : <Clock3 className="w-8 h-8 text-amber-500" />}
          </div>
          <h1 className="font-display font-extrabold text-2xl text-slate-900 mb-2">
            {isAdmin ? 'Successfully upgraded your account' : 'Pending'}
          </h1>
          <p className="text-slate-500 text-sm max-w-md mx-auto">
            {isAdmin
              ? 'Your account has been successfully upgraded to Pro.'
              : 'Your upgrade request has been submitted and is pending verification. We will notify you once it has been reviewed.'}
          </p>
        </div>
      </div>
    );
