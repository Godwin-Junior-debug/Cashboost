...
export default function Dashboard({ onNavigate }: DashboardProps) {
  ...
  async function requestWithdrawal(e: React.FormEvent, code: string = '') {
    e.preventDefault();
    if (!user || !profile) return;
  ...
  // NOTE: requires a `voucher-receipts` storage bucket and a `pro_payment_requests`
  ...
  async function submitPaymentProof(details: PaymentConfirmDetails) {
    ...
    try {
      ...
      if (profile?.is_admin) {
        ...
        showToast('success', 'Your account has been successfully upgraded to Pro.');
      } else {
        showToast('success', 'Your upgrade request has been submitted and is pending verification.');
      }
      setPaymentSubmitted(true);
    } catch (err: unknown) {
      const supabaseError = err as { message?: string; code?: string; details?: string; hint?: string };
      const message = supabaseError.message
        ? `${supabaseError.message}${supabaseError.code ? ` (${supabaseError.code})` : ''}`
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="grid grid-cols-1 lg:grid-cols-[240px_1fr] gap-6">
          ...
          <main className="min-w-0 pb-28 lg:pb-0">
            ...
            {tab === 'upgrade' && (
              <UpgradeProPage
                isPro={profile?.is_admin ? false : profile?.is_pro}
                actionLoading={actionLoading}
