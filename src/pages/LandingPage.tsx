...
import {
  ...
  Quote, ChevronDown, Smartphone, Clock,
} from 'lucide-react';
...
const testimonials = [
  { name: 'Chidi Okafor', role: 'Student, UNILAG', text: 'I made my first ₦15,000 in two weeks just doing tasks between lectures. Cashboost9ja is the real deal — no scams, just real work for real pay.', rating: 5, avatar: 'C' },
  ...
  { name: 'Fatima Ibrahim', role: 'Content Creator', text: 'The content rewards are insane. One TikTok video earned me ₦12,000 in a single day. Cashboost9ja pays creators what they deserve.', rating: 5, avatar: 'F' },
  { name: 'Emeka Nwosu', role: 'NYSC Corps Member', text: 'As a corps member, allowance is small. Cashboost9ja helped me earn extra during my service year. I have withdrawn over ₦80,000 so far.', rating: 5, avatar: 'E' },
...
const faqs = [
  { q: 'Is Cashboost9ja legit?', a: 'Yes. Cashboost9ja has paid out over ₦185M to members across Nigeria. Every payment is for work you actually complete, and you can see real member reviews above.' },
    ...
    <div className="overflow-hidden">
      ...
      <section className="relative min-h-screen flex items-center pt-16 pb-20 bg-gradient-to-br from-slate-950 via-primary-950 to-slate-900">
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 grid lg:grid-cols-2 gap-12 items-center">
          ...
          {/* Hero visual — passbook ledger, not an app-dashboard mockup */}
          <div className="relative hidden lg:block animate-scale-in" style={{ animationDelay: '0.3s' }}>
            <div className="relative rotate-1 hover:rotate-0 transition-transform duration-500">
              <div className="absolute inset-0 bg-gradient-to-br from-primary-500/30 to-accent-500/30 rounded-2xl blur-2xl" />

              {/* Perforated passbook card */}
              <div className="relative bg-[#fdfcf7] rounded-2xl shadow-2xl overflow-hidden border border-black/5">
                {/* Perforation strip */}
                <div className="absolute left-0 top-0 bottom-0 w-6 flex flex-col justify-evenly items-center bg-primary-950/[0.03]">
                  {Array.from({ length: 14 }).map((_, i) => (
                    <span key={i} className="w-2 h-2 rounded-full bg-[#0b0f14]/10" />
                  ))}
                </div>
                ...
                <div className="pl-10 pr-6 py-6">
                  <div className="flex items-start justify-between mb-5">
                    <div>
                      <p className="font-mono text-[11px] tracking-widest text-primary-700/70 uppercase">Savings Passbook</p>
                      <p className="font-display font-extrabold text-lg text-slate-900">Cashboost9ja</p>
                    </div>
                    <div className="rotate-6 border-2 border-accent-600/70 text-accent-700 rounded-md px-2 py-1 font-mono text-[10px] font-bold tracking-widest uppercase">
                      Verified
                    </div>
                  </div>

                  <div className="border-y border-dashed border-slate-300 py-4 mb-4">
                    <p className="font-mono text-[11px] tracking-widest text-slate-500 uppercase mb-1">Running Balance</p>
                    <p className="font-mono font-bold text-4xl text-slate-900">₦42,850<span className="text-xl text-slate-400">.00</span></p>
                    <div className="flex items-center gap-1 mt-1.5 text-primary-700 text-xs font-mono">
                  ...
                  <table className="w-full font-mono text-xs">
                    <thead>
                      <tr className="text-slate-400 uppercase tracking-wider">
                        ...
                        <th className="text-right font-medium pb-2">Amt</th>
                      </tr>
                    </thead>
                    <tbody className="text-slate-700">
                      {[
                        { desc: 'Twitter follow task', amount: '+₦150', time: '2m ago' },
                        { desc: 'Referral — Amaka E.', amount: '+₦500', time: '1h ago' },
                        { desc: 'YouTube subscribe', amount: '+₦200', time: '3h ago' },
                      ].map((t, i) => (
                        <tr key={i} className="border-t border-dotted border-slate-200">
                          <td className="py-2">
                            <p className="text-slate-800">{t.desc}</p>
                            <p className="text-slate-400 text-[10px]">{t.time}</p>
                          </td>
                          <td className="py-2 text-right text-primary-700 font-bold align-top">{t.amount}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
                  ...
                  <div className="flex items-center gap-2 mt-5 pt-4 border-t border-slate-200">
                    ...
                    <p className="text-[11px] font-mono text-slate-500">CAC registered · Bank-grade encryption</p>
                  </div>
                </div>
              </div>

              {/* Floating stamp badge */}
              <div className="absolute -top-5 -right-5 glass rounded-2xl px-4 py-3 animate-float">
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    ...
                    <p className="text-slate-400 text-xs font-mono">₦10,000</p>
                  </div>
                </div>
              </div>
            </div>
      ...
      {/* ===== STATS LEDGER ===== */}
      <section ref={statsRef} className="py-14 bg-[#fdfcf7] border-y border-slate-200">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-2 lg:grid-cols-4 divide-x divide-y lg:divide-y-0 divide-dashed divide-slate-300 border border-dashed border-slate-300 rounded-xl overflow-hidden">
            {stats.map((s, i) => (
              <div
                key={s.label}
                className={`flex items-center gap-3 px-6 py-5 transition-all duration-700 ${statsVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}
                style={{ transitionDelay: `${i * 100}ms` }}
              >
                <s.icon className="w-5 h-5 text-primary-600 flex-shrink-0" />
                <div>
                  <p className="font-mono font-bold text-2xl sm:text-3xl text-slate-900 leading-none">
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="max-w-3xl mx-auto">
            {steps.map((step, i) => (
              <div
                key={step.num}
                className={`relative flex gap-6 group transition-all duration-700 ${howVisible ? 'opacity-100 translate-x-0' : 'opacity-0 -translate-x-8'}`}
                style={{ transitionDelay: `${i * 150}ms` }}
              >
                <div className="flex flex-col items-center">
                  <div className="w-14 h-14 rounded-2xl bg-gradient-to-br from-primary-600 to-accent-500 flex items-center justify-center shadow-glow group-hover:scale-110 transition-transform duration-300 flex-shrink-0 relative z-10">
                    <step.icon className="w-7 h-7 text-white" />
                  </div>
                  {i < steps.length - 1 && (
                    <div className="w-px flex-1 bg-gradient-to-b from-primary-300 to-slate-200 my-2 min-h-[3rem]" />
                  )}
                </div>

                <div className={`flex-1 bg-white rounded-2xl p-6 border border-slate-200 hover:border-primary-300 hover:shadow-lg transition-all duration-300 ${i < steps.length - 1 ? 'mb-6' : ''}`}>
                  <div className="flex items-center gap-3 mb-2">
                    <span className="font-mono text-xs font-bold text-primary-600 bg-primary-50 rounded px-2 py-0.5">STEP {step.num}</span>
                  </div>
                  <h3 className="font-display font-bold text-xl text-slate-900 mb-2">{step.title}</h3>
      ...
      <section id="earn" ref={earnRef} className="py-24 bg-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="max-w-4xl mx-auto rounded-2xl border border-slate-200 divide-y divide-slate-200 overflow-hidden bg-white">
            {earningMethods.map((m, i) => (
              <div
                key={m.title}
                className={`group flex flex-col sm:flex-row sm:items-center gap-4 p-6 hover:bg-slate-50 transition-all duration-500 ${earnVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}
                style={{ transitionDelay: `${i * 100}ms` }}
              >
                <div className={`w-14 h-14 rounded-2xl ${m.bg} flex items-center justify-center flex-shrink-0 group-hover:scale-110 transition-transform duration-300`}>
                  <m.icon className={`w-7 h-7 ${m.accent}`} />
                </div>

                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 flex-wrap mb-1">
                    <h3 className="font-display font-bold text-lg text-slate-900">{m.title}</h3>
                    <span className={`text-[11px] font-semibold px-2 py-0.5 rounded-full ${m.bg} ${m.accent}`}>
                      {m.tagline}
                    </span>
                  </div>
                ...
                <div className="flex sm:flex-col items-center sm:items-end justify-between sm:justify-center gap-2 sm:w-40 flex-shrink-0 pt-3 sm:pt-0 border-t sm:border-t-0 sm:border-l border-slate-100 sm:pl-6">
                  <span className="font-mono font-bold text-slate-900 text-sm sm:text-right">{m.range}</span>
                  <button
                    onClick={() => onNavigate('register')}
                    className="flex items-center gap-1 text-sm font-semibold text-primary-600 hover:text-primary-700 group-hover:gap-2 transition-all whitespace-nowrap"
                  >
      ...
      <section id="reviews" ref={reviewsRef} className="py-24 bg-slate-50 relative overflow-hidden">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-16">
            ...
            <p className="text-lg text-slate-600 max-w-2xl mx-auto">
              See what earning with Cashboost9ja looks like for Nigerians across the country.
            </p>
          </div>
